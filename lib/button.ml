...
type t = {
  ...
  action : string -> unit; (* The callback function taking an int parameter *)
  action_param : string; (* The parameter to pass to the callback *)
}
...
let is_pressed { rect; _ } ~mouse_x ~mouse_y =
  ...
  in
  if pressed then print_endline "pressed";
  pressed
...
let handle_event button event =
  ...
  | `Mouse_button_down ->
      ...
      if is_pressed button ~mouse_x:x ~mouse_y:y then
        button.action button.action_param
