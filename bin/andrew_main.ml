...
let render_player renderer game_map game_state =
  ...
  let texture =
    ...
    | South -> load_texture_from_file renderer "textures/pc-front-96.png"
    | North -> load_texture_from_file renderer "textures/pc-back-96.png"
    | West -> load_texture_from_file renderer "textures/pc-left-96.png"
    | East -> load_texture_from_file renderer "textures/pc-right-96.png"
  in
  match Sdl.render_copy ~src ~dst renderer texture with
  | Ok () -> () (* Successfully rendered, do nothing *)
  | Error (`Msg e) -> Sdl.log "Failed to render tile: %s" e
    ...
    (fun y row ->
      Array.iteri
        (fun x tile ->
          if tile.deco1 then
            let tree_texture =
              load_texture_from_file renderer "textures/tree1.png"
            in
            let src =
